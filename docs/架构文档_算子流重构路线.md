...
| P2 流程编辑 UI | ✅ 完成 | `ui/flow_editor.py`：节点库拖拽、画布连线、参数面板、方案保存/加载、单图运行（Tools → Flow Editor...）；可视化节点 `heatmap_overlay`(Grad-CAM)/`heightmap_rainbow` 就绪，标准方案 FileSource→ClsInfer→HeatmapOverlay 可复跑 |
...
| W3 | ✅ 流程编辑器（拖拽+连线+参数+保存/加载，`ui/flow_editor.py`）；✅ 热力图节点 `heatmap_overlay`（彩虹图 `heightmap_rainbow` 已有） |
